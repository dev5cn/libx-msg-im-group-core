...
void XmsgNe::estab()
{
	...
	req->set_neg(X_MSG_IM_GROUP);
	req->set_cgt(XmsgImGroupCfg::instance()->cfgPb->cgt());
	req->set_salt(Crypto::gen0aAkey256());
	req->set_sign(Crypto::sha256ToHexStrLowerCase(XmsgImGroupCfg::instance()->cfgPb->cgt() + req->salt() + this->pwd));
	req->set_alg(this->alg);
	if (this->neg == X_MSG_IM_HLR)
	{
		XmsgMisc::insertKv(req->mutable_ext(), "sub-x-msg-client-estb-event", "enable"); 
...
void XmsgNe::authOk(shared_ptr<XmsgNeUsr> nu, shared_ptr<XmsgNeAuthRsp> rsp)
{
